...
def generate_error_vectors_independent(config, num_error_vectors_to_generate):
    """
...
def generate_error_vectors_associated(config, num_error_vectors_to_generate):
    """
    An error vector is a succinct description of how corruptions will be introduced
    into an original, master record
    For each original record, multiple error vectors can be generated,
    ...
    # The following is an extremely simple implementation trying to introduce correlation!
...
# Applying Error Vector to corrupt a record. Should work as it is.
