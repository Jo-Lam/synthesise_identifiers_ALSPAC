...
class RecordCorruptor:
    ...
    def probability_adjustment_tuples(self, record):
        ...
        for pa in self.probability_adjustments:
            new_tuples = ProbabilityAdjustmentFromLookup.get_adjustment_tuples(pa, record)
            tuples.extend(new_tuples)

        return tuples

    def apply_probability_adjustments(self, record):
        print("run: apply probability adjustment")
        # print(self.probability_adjustment_tuples(record))
        # type(self.probability_adjustment_tuples(record))
        for c in self.corruptions:
            CompositeCorruption.reset_probability(c)
        for corruption, bayes_factor in self.probability_adjustment_tuples(record):
            CompositeCorruption.adjust_probability_using_bayes_factor(corruption, bayes_factor)
